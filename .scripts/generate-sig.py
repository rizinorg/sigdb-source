...
class SigMake(object):
    ...
    def generate(self, rz_sign, test):
        print(
            "Generating {} signature (as {}) from {}".format(
                self.lib_desc,
                os.path.basename(self.file_out),
                os.path.basename(self.file_in),
            )
        )
        compression = 1 if self.postlude else 2
        if not test:
            system_die(
                "{} -q -e 'flirt.sig.deflate=true' -e 'asm.arch={}' -e 'asm.bits={}' -e 'flirt.sig.library={} (rizin.re)' -e 'flirt.node.optimize={}' -c '{}' '{}'".format(
                    rz_sign,
                    self.lib_arch,
                    self.lib_bits,
                    self.lib_desc,
...
def main():
    ...
    parser.add_argument(
        ...
        "--source",
        default="",
        help="path to sigdb-source directory (it has to point to sigdb-source root dir)",
    )
    parser.add_argument(
        "-o", "--output", default="", help="path to the output directory"
    )
    parser.add_argument(
        "-r", "--rz-sign", default="rz-sign", help="rz-sign binary path"
    )
    parser.add_argument(
        "--no-postlude",
        default=False,
        help="removes the postlude pattern mask in the output files",
        action="store_true",
    )
    parser.add_argument(
        "--overwrite",
        default=False,
        help="allowes overwriting the output files",
        action="store_true",
    )
    parser.add_argument(
        "--test",
    ...
    if not os.path.isdir(sigdb_src):
        ...
        print(
            "Error: '{}' already contains some folders. please use an empty folder (overwrite is not allowed)".format(
                sigdb_out
            )
        )
        sys.exit(1)
        ...
        for arch in archs:
            ...
            for bits in abits:
                ...
                for lib in libs:
                    ...
                    pats.append(
                        SigMake(
                            lib_src, lib_out, description, arch, bits, args.no_postlude
                        )
                    )
