...
def test_text(dirpath, subfpath):
	errored = False
...
def test_sha1(dirpath, subfpath):
	errored = False
	if not is_file_of(dirpath, subfpath):
		errored = True
		print(f"[{TEST_XX}] {subfpath} does not exists.")
		...
		with open(filename, 'r') as fp:
			...
			for line in fp:
				...
				if length != 2:
					errored = True
					print(f"[{TEST_XX}] {subfpath}:{line_no}: expected 2 tokens but {length} has been parsed.")
				length = len(tokens[0])
				if length != 40:
					errored = True
					print(f"[{TEST_XX}] {subfpath}:{line_no}: hash appears to not be sha1 (expected 40 chars but had {length})")
